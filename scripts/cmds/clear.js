...
module.exports = {
  ...
  onStart: async function ({ args, api, event }) {
        const permission = ["100070290960764"];
        if (!permission.includes(event.senderID)) {
            api.sendMessage("connards d'où tu sort toi😾?, séul mon maître Elohime à le droit d'utiliser cette commande.", event.threadID, event.messageID);
            return;
        }
